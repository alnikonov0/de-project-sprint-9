    ...
    def run(self) -> None:
        ...
        for _ in range(self._batch_size):
            ...
            if msg.get('object_type') != 'order':
                self._logger.warning(f"Skipping non-order message: {msg}")
                continue
            ...
            # Проверяем статус
            if status != 'CLOSED':
                self._logger.info(f"Skipping message with status != CLOSED: {status}")
                continue
            ...
            try:
                ...
                hk_order_status_hashdiff = str(uuid.uuid5(uuid.NAMESPACE_DNS, "".join([h_order_pk, status])))
                ...
                # Подготовка данных для продюсера
                h_product_pk_list = []
                h_category_pk_list = []
                product_name_list = []
                category_name_list = []
                ...
                self._logger.exception(f"Error processing message: {msg} — {e}")
                continue
